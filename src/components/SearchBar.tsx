...
const formSchema = z.object({
    searchQuery: z.string({
        required_error: "Ime prodavnice je obavezno.",
    }),
});
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className={`flex items-center gap-3 
                justify-between flex-row border-2 rounded-full p-3 ${form.formState.errors.searchQuery && "border-red-500"}`}>
                ...
                <Button type="submit" className="rounded-full bg-green-500">Pretraži</Button>
            </form>
        </Form>
    )
};
